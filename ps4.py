...
class SimpleBacteria(object):
    ...
    def reproduce(self, pop_density):
        ...
        if reproduce_prob <= self.birth_prob * (1-pop_density):
...
def calc_pop_avg(populations, n):
    ...
    calc_avg = pop_array.mean(axis=0)  # axis 0 is column of matrix
    return calc_avg[n]
...
def simulation_without_antibiotic(num_bacteria,
    ...
    # Initialize bacteria population list across trials
    pop = []
    
    for trial in range(num_trials):
        
        # Intialize bacteria list in each trial
        bacteria = []

        # Instantiate bacteria population for the patient
        ...
        # Initialize bacteria population in each trial
        pop_per_trial = []
        
        # Update bacteria population for each timestep. Timestep 0 has starting number of bacteria
        for timestep in range(300):
            if timestep == 0:
                pop_per_trial.append(patient.get_total_pop())
            else:
                pop_per_trial.append(patient.update())
        
        # Append bacteria population in each trial to population across trials
        pop.append(pop_per_trial)
...
# y_coords = []
# for timestep in range(300):
#     x_coords.append(timestep)
#     y_coords.append(calc_pop_avg(populations, timestep))
    ...
    def reproduce(self, pop_density):
        ...
        if self.birth_prob * (1-pop_density) >= random.random():
            if self.get_resistant():
                ...
                resistant_mut_prob = self.mut_prob * (1-pop_density)
                if resistant_mut_prob >= random.random():
    ...
    def update(self):
        ...
        for bacteria in self.bacteria:
            if bacteria.is_killed():
                ...
                if self.on_antibiotic and not bacteria.get_resistant():
                        self.bacteria.remove(bacteria)
                else:                    
                    pop_density = self.get_total_pop() / self.max_pop
...
for timestep in range(400):
    x_coords.append(timestep)
    y_coords1.append(calc_pop_avg(total_pop, timestep))
    y_coords2.append(calc_pop_avg(resistant_pop, timestep))
...
print('Simulation A Total Population 95% CI:', calc_95_ci(total_pop, 299))
print('Simulation A Resistant Population 95% CI:', calc_95_ci(resistant_pop, 299))